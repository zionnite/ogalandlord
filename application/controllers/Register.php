...
class Register extends My_Controller {
	...
	public function reg_prom($props_id=NULL, $url_code=NULL){
        $data['alert']			=$this->session->flashdata('alert');

        $data['props_id']       = $props_id;
        $data['url_code']       = $url_code;
		$data['content'] ='auth/signup_prom';
		$this->load->view($this->auth_layout,$data);
	}


    public function reg_prom_insert(){
        if($this->input->post('submit')){	
			...
			$status               		=$this->input->post('status');
			$phone               		=$this->input->post('phone');
			$url_code               	=$this->input->post('url_code');
			...
			if($this->form_validation->run() == FALSE){
                ...
                $action     =$this->Users_db->register_user_3($full_name,$email,$pass,$status,$phone,$user_name,$url_code, $props_id,'logo.png');
                if($action){
                    $this->success_alert_callbark('Account Created... An email has been sent to you, please click on the link to verify your email');
                    $user_id        = $this->Users_db->get_user_id_by_email($email);
                    $title  ='Confirm Account';
