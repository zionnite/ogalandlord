	...
	public function get_webhooks(){
        ...
        if($event == 'charge.success' && $status =='success'){
            ...
            if($type == 'agent_rc'){
                ...
                $this->Transaction_db->update_complete_transaction($reciever_id, $props_id);
                $this->Wallet_db->fund_total_earning($reciever_id,-$amount);
            }
            else if($type =='insur_rc'){
                //credit insurance earning
              
                $this->Transaction_db->update_complete_transaction('admin', $props_id);
                $this->Wallet_db->fund_insurance_earning(-$amount);

                //credit site earning
                
                $this->Transaction_db->update_complete_transaction('admin', $props_id);
                $this->Wallet_db->remove_from_site_earning($sender_id,$reciever_id,$props_id);
            }
            else if($type =='ref_rc'){

                $referal_id           = $dataarray['data']['recipient']['metadata']['referal_id'];

                $this->Transaction_db->update_complete_transaction($referal_id, $props_id);
                $this->Wallet_db->fund_total_earning($referal_id, -$amount);
            }
            else if($type =='prom_rc'){
                ...
                $this->Transaction_db->update_complete_transaction($promoter_id, $props_id);
                $this->Wallet_db->fund_total_earning($promoter_id,-$amount);
